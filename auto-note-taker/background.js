...
async function beginGoogleLogin() {
  ...
  // Use the default extension redirect URI for easier Supabase setup.
    ...
    chrome.identity.launchWebAuthFlow(
      ...
      (redirectedTo) => {
        if (chrome.runtime.lastError) {
          reject(
            new Error(
              `${chrome.runtime.lastError.message}. Make sure this redirect URL is added in Supabase Auth settings: ${redirectUri}`
            )
          );
          return;
        }
  ...
  const hashParams = new URLSearchParams((callbackUrl.split("#")[1] || "").trim());
  const queryParams = new URLSearchParams((callbackUrl.split("?")[1] || "").trim());
  const accessToken = hashParams.get("access_token") || queryParams.get("access_token");

  if (!accessToken) {
    throw new Error(
      `Login failed: access token missing. Callback URL was: ${callbackUrl}`
    );
  }
